...
namespace l1pf_calo {
    ...
    class SingleCaloClusterer {
        public:
            ...
            void add(const l1tpf::Particle &particle) { rawet_(particle.eta(), particle.phi()) += particle.pt(); }
            void run() ; 
    ...
    class SimpleCaloLinker {
        public:
            ...
            std::vector<l1tpf::Particle> fetch() const ;
        private:
            std::unique_ptr<Grid> grid_;
...
#if 0
void doEcalHCalLink() {
    // Mark as hcal linked any ecal cluster that has an HCal cluster within 1 unit
    // should we do the same energy sharing as we do within the clustering, in case there's an ECal
    // cluster right in between two HCal clusters?
    for (Cell &c : allcells()) {
        if (c.calo[em].clust.pt > 0) {
            if (c.calo[had].clust.pt > 0) {
                c.ecal_linked_to_hcal_same = true;
            } else {
                c.sum_neigh_hcal_clust_pt = 0;
                for (const Cell * n : c.neighs) {
                    if (n->calo[had].clust.pt > 0) {
                        c += sum_neigh_hcal_clust_pt = n->calo[had].clust.pt;
                        c.ecal_linked_to_hcal_neigh = true;
                    }
                }
            }
        }
    }
    // Make HCal + ECal clusters if 
